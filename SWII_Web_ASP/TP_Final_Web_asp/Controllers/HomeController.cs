...
namespace Web_TP_Final.Controllers
{
    public class HomeController : Controller
    {
        public async Task<IActionResult> Index()
        {
            ...
            {
                using (var response = await httpClient.GetAsync("https://localhost:44354/api/v1/produtos"))
                {
        ...
        public async Task<IActionResult> AddProduto(Produto produto)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.PostAsync("https://localhost:44354/api/v1/produtos", content))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> DeleteProduto(int id)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                client.DeleteAsync(String.Format("{0}/{1}", "https://localhost:44354/api/v1/produtos", id));
        ...
        public async Task<IActionResult> UpdateProdutoById(int id)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("https://localhost:44354/api/v1/produtos/" + id))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateProduto(int id, Produto produto)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                HttpResponseMessage responseMessage = await httpClient.PutAsync("https://localhost:44354/api/v1/produtos/" + produto.Id, content);
                if (responseMessage.IsSuccessStatusCode)
                {
                    return RedirectToAction("Index");
                }
